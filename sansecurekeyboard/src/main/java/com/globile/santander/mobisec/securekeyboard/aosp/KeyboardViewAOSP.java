package com.globile.santander.mobisec.securekeyboard.aosp;/*
...
/**
 * A view that renders a virtual {@link KeyboardAOSP}. It handles rendering of keys and
...
public class KeyboardViewAOSP extends View implements View.OnClickListener {
    ...
    private static final int[] KEY_DELETE = {KeyboardAOSP.KEYCODE_DELETE};
    ...
    private Map<KeyboardAOSP.Key, View> mMiniKeyboardCache;
    private KeyboardAOSP.Key[] mKeys;
    ...
    private KeyboardAOSP.Key mInvalidatedKey;
    ...
    public KeyboardViewAOSP(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.keyboardViewStyle);
    }

    public KeyboardViewAOSP(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initView(context, attrs, defStyleAttr, 0);
    }

    public KeyboardViewAOSP(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr);
        initView(context, attrs, defStyleAttr, 0);
    ...
    private void initView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.KeyboardViewAOSP, defStyleAttr, defStyleRes);
        ...
        // mAccessibilityManager = AccessibilityManager.getInstance(context); - INTERNAL CALLING
    ...
    public void setKeyboard(KeyboardAOSP keyboard) {
        ...
        removeMessages();
        mKeyboard = keyboard;
        List<KeyboardAOSP.Key> keys = mKeyboard.getKeys();
        mKeys = keys.toArray(new KeyboardAOSP.Key[keys.size()]);
        requestLayout();
     ...
     * @see KeyboardViewAOSP#isShifted()
     */
     ...
     * @see KeyboardViewAOSP#setShifted(boolean)
     */
    ...
    private void computeProximityThreshold(KeyboardAOSP keyboard) {
        ...
        final KeyboardAOSP.Key[] keys = mKeys;
        if (keys == null) return;
        int length = keys.length;
        int dimensionSum = 0;
        for (int i = 0; i < length; i++) {
            KeyboardAOSP.Key key = keys[i];
    ...
    private void onBufferDraw() {
        ...
        final KeyboardAOSP.Key[] keys = mKeys;
        final KeyboardAOSP.Key invalidKey = mInvalidatedKey;
        ...
        for (int i = 0; i < keyCount; i++) {
            final KeyboardAOSP.Key key = keys[i];
    ...
    private int getKeyIndices(int x, int y, int[] allKeys) {
        ...
        for (int i = 0; i < keyCount; i++) {
            final KeyboardAOSP.Key key = keys[nearestKeyIndices[i]];
            int dist = 0;
    ...
    private void detectAndSendKey(int index, int x, int y, long eventTime) {
        if (index != NOT_A_KEY && index < mKeys.length) {
            final KeyboardAOSP.Key key = mKeys[index];
            if (key.text != null) {
                ...
                if (mInMultiTap) {
                    if (mTapCount != -1) {
                        mKeyboardActionListener.onKey(KeyboardAOSP.KEYCODE_DELETE, KEY_DELETE);
                    } else {
                        mTapCount = 0;
                    }
     ...
     */
    private CharSequence getPreviewText(KeyboardAOSP.Key key) {
        if (mInMultiTap) {
            // Multi-tap
    ...
    private void showPreview(int keyIndex) {
        ...
        if (oldKeyIndex != mCurrentKeyIndex) {
            if (oldKeyIndex != NOT_A_KEY && keys.length > oldKeyIndex) {
                KeyboardAOSP.Key oldKey = keys[oldKeyIndex];
            ...
            if (mCurrentKeyIndex != NOT_A_KEY && keys.length > mCurrentKeyIndex) {
                KeyboardAOSP.Key newKey = keys[mCurrentKeyIndex];
                newKey.onPressed();
    ...
    private void showKey(final int keyIndex) {
        ...
        KeyboardAOSP.Key key = keys[keyIndex];
        if (key.icon != null) {
        ...
        if (mAccessibilityManager.isEnabled()) {
            ...
            switch (code) {
                ...
                case KeyboardAOSP.KEYCODE_DONE:
                    ...
                    break;
                case KeyboardAOSP.KEYCODE_MODE_CHANGE:
     ...
     * may not be honored.
     *
     * @param keyIndex the index of the key in the attached {@link KeyboardAOSP}.
    ...
    public void invalidateKey(int keyIndex) {
        ...
        final KeyboardAOSP.Key key = mKeys[keyIndex];
        mInvalidatedKey = key;
    ...
    private boolean openPopupIfRequired(MotionEvent me) {
        ...
        KeyboardAOSP.Key popupKey = mKeys[mCurrentKey];
    ...
    protected boolean onLongPress(KeyboardAOSP.Key popupKey) {
        ...
        if (popupKeyboardId != 0) {
            ...
            if (mMiniKeyboardContainer == null) {
                ...
                mMiniKeyboard = (KeyboardViewAOSP) mMiniKeyboardContainer.findViewById(
                        R.id.keyboardView);
    ...
    private boolean repeatKey() {
        KeyboardAOSP.Key key = mKeys[mRepeatKeyIndex];
    ...
    private void checkMultiTap(long eventTime, int keyIndex) {
        ...
        KeyboardAOSP.Key key = mKeys[keyIndex];
